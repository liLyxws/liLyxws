
<p align="center">
  <img src="https://raw.githubusercontent.com/liLyxws/liLyxws/main/sqa.gif" width="1000">
  <img src="https://raw.githubusercontent.com/liLyxws/liLyxws/main/avt.gif" width="250">
<br>
  <a href="https://www.linkedin.com/feed/">
    <img src="https://img.shields.io/badge/-Linkedin-blue?style=flat-square&logo=Linkedin&logoColor=white" />
  </a>

  <a href="https://github.com/liLyxws">
    <img src="https://img.shields.io/github/followers/liLyxws?label=follow&style=social" />
  </a>
</p>


### <img src="https://media.giphy.com/media/VgCDAzcKvsR6OM0uWg/giphy.gif" width="50">✨ Get to Know Me a Little

```javascript
const Ly = {
  pronouns: ["she", "her"],
  status: "BSIS Student",
  role: "Software Quality Assurance (aspiring)",

  // current experience
  code: [, HTML, CSS, PHP, Javascript, Python],

  tools: ["React", "Laravel"],
  
  // learning roadmap (future goals)
  goals: [
    "Level up automation testing",
    "Build cool React projects",
    "Explore JavaScript deeply"
  ],

  // passions 
  interests: {
    sqa: ["Performance Testing"],
    frontendDev: ["UI/UX basics", "Responsive Design"]
  },

  progress: {
    currentLevel: "Beginner",
    lookingForward: "Exploring QA + Frontend Dev"
  }

}



```

> *It is with great delight that I present myself to you.*

---


<details>
  <summary> <img width="25" height="27" alt="bar" src="https://github.com/user-attachments/assets/8464fe47-1e28-4ddb-b9c7-93fd7efa4bc7" /> Github Stats</summary> 
  
  <p align="center">
    <img src="https://streak-stats.demolab.com/?user=liLyxws&theme=gotham&hide_border=false" />
    &nbsp;&nbsp;&nbsp;
    <img src="https://github-readme-stats.shion.dev/api/top-langs/?username=liLyxws&theme=gotham&hide_border=false&include_all_commits=true&count_private=true&layout=compact" />
  </p>
</details>

---
<p align="center">
  <img width="65" height="54" alt="cat" src="https://github.com/user-attachments/assets/847f21a6-9ddc-4791-91e7-8f02663e78ea" />
  <br>
  <em><b>
     "Fall seven times, stand up eight."
  </b></em>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=liLyxws&label=Profile%20views&color=000000&style=for-the-badge" alt="liLyxws" />
</p>


